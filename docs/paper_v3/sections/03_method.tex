...
the trajectory-anchor extraction and the calibrated-utility ranking
...
base ranking $\tilde{\mathcal{R}}_i^{(t)}$ (Eq.~\ref{eq:calibrated}
below) with a stability-controlled structural factor, each at the
...
transform of the composite reward into a bounded calibrated utility
(``Calibrated utility''), which serves as the carrier signal that
...
\paragraph{Calibrated utility (pool-normalised composite reward).}
The composite reward $\mathcal{R}_i^{(t)}$ in
Eq.~\ref{eq:composite} carries a task-mix-dependent scale
(Sec.~\ref{sec:method-motivation}). We mitigate this by passing it
through a deterministic pool-level z-score followed by a logistic
map, yielding a bounded \emph{calibrated utility}
\begin{equation}
  \tilde{\mathcal{R}}_i^{(t)}
  :=
  \sigma\!\left(
    \frac{\mathcal{R}_i^{(t)} - \bar{\mathcal{R}}^{(t)}}
         {\sigma_{\mathcal{R}}^{(t)} + \varepsilon}
  \right)
  \;\in\; (0,1),
  \label{eq:calibrated}
\end{equation}
where $\bar{\mathcal{R}}^{(t)}$ and $\sigma_{\mathcal{R}}^{(t)}$
are the pool-level mean and standard deviation of
$\{\mathcal{R}_i^{(t)}\}_{i=1}^N$ at epoch~$t$, $\varepsilon>0$ is
a small numerical stabiliser ($10^{-6}$), and $\sigma(\cdot)$ is
the logistic function. Centering removes the additive
task-mix-dependent baseline that motivates the structural anchor
(Eq.~\ref{eq:total-variance}); division by
$\sigma_{\mathcal{R}}^{(t)}$ controls cross-task scale drift; and
the logistic mapping renders $\tilde{\mathcal{R}}_i^{(t)}$ bounded
and non-negative so it composes multiplicatively with the anchor
factor below. The transform is deterministic: it adds only two
pool-level scalar reductions per refresh step and requires no
learned parameters, policy, or critic.

\paragraph{Selection score.}
\textsc{TADS} forms the final per-candidate ranking score by
multiplying the calibrated utility by the trajectory-anchor factor:
\begin{equation}
\boxed{
s_i^{(t)}
=
\underbrace{\tilde{\mathcal{R}}_i^{(t)}}_{\substack{\text{calibrated-utility factor} \in (0,1)\\[1pt]
...
\label{eq:score}
\end{equation}
with hyperparameter $\lambda \ge 0$. The multiplicative form
carries three useful properties. \emph{(i) Clean ablation:}
$\lambda{=}0$ recovers the calibrated-utility base ranking
$\tilde{\mathcal{R}}_i^{(t)}$ exactly; any gain at $\lambda>0$ is
attributable to the trajectory anchor. \emph{(ii) Variance-scale
decoupling:} the calibrated-utility factor still inherits any
residual task-mix drift of Eq.~\ref{eq:total-variance} that
survives centering; the anchor factor is a bounded pool-level PCA
statistic that acts as a relative modulation rather than an
additive score---a separation that additive composition would
...
per epoch; the same pool-level statistics are reused for
$\bar{\mathcal{R}}^{(t)}, \sigma_{\mathcal{R}}^{(t)}$ in
Eq.~\ref{eq:calibrated}, so the calibrated utility adds no
...
\FOR{$t = 1, \dots, T$}
  ...
  \STATE Compute pool-level $w$ and composite reward
         $\mathcal{R}_i$
         \hfill (Eqs.~\ref{eq:w},~\ref{eq:composite})
  \STATE Compute pool moments
         $\bar{\mathcal{R}}^{(t)}, \sigma_{\mathcal{R}}^{(t)}$ and
         calibrated utility $\tilde{\mathcal{R}}_i^{(t)}$
         \hfill (Eq.~\ref{eq:calibrated})
  \STATE Score
         $s_i^{(t)} \leftarrow
          \tilde{\mathcal{R}}_i^{(t)} \cdot
...
for the \textsc{TADS} criterion is not specific to the calibrated
utility of Eq.~\ref{eq:calibrated}. Empirical verification across
1{,}496 layer-refresh measurements (App.~\ref{app:thm-verification})
confirms the bound is non-vacuous.
