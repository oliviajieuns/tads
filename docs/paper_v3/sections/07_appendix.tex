...
s_i^{(t)} = B_i^{(t)}A_i^{(t)},
\qquad
B_i^{(t)} := \tilde{\mathcal{R}}_i^{(t)} \in (0,1),
\qquad
A_i^{(t)} := 1+\lambda\widetilde{\mathrm{align}}_i^{(t)} .
\]
Here $B_i^{(t)}$ is the calibrated-utility base ranking
(Eq.~\ref{eq:calibrated}) and $A_i^{(t)}\in[1,1+\lambda]$ is the
anchor factor.
...
$B_i^{(t)}$; for the calibrated utility of Eq.~\ref{eq:calibrated}
the bound holds with $B_{\max}=1$ since
$\tilde{\mathcal{R}}_i^{(t)} \in (0,1)$. Thus the stability result
concerns the trajectory anchor and its multiplicative factor; it is
not tied to the particular calibrated-utility transform used in our
implementation.
...
in both Eq.~\ref{eq:w} and Eq.~\ref{eq:calibrated}.

\paragraph{Composite reward and calibrated utility.}
At each refresh step we form
$\mathcal{R}_i^{(t)} = w^{(t)}\mathcal{L}_i^{(t)} +
(1-w^{(t)})\mathcal{H}_i^{(t)}$
with $w^{(t)}$ from Eq.~\ref{eq:w} computed pool-wide, and obtain
the calibrated utility
$\tilde{\mathcal{R}}_i^{(t)} = \sigma\bigl((\mathcal{R}_i^{(t)} -
\bar{\mathcal{R}}^{(t)})/(\sigma_{\mathcal{R}}^{(t)} +
\varepsilon)\bigr)$
via Eq.~\ref{eq:calibrated}. The transform is deterministic, adds
no learned parameters, and reuses the pool-level statistics already
computed for $w^{(t)}$.
...
reported numbers use the deterministic calibrated-utility ranking
of Eq.~\ref{eq:score}.
...
$\lambda=0$ calibrated-utility base, while large $\lambda$
saturates.
