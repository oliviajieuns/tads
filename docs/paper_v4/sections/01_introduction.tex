...
the per-candidate score as the product of the composite reward
and a trajectory-anchor factor:
\[
s_i^{(t)}
=
\underbrace{\mathcal{R}_i^{(t)}}_{\substack{\text{composite reward}\\[1pt]\text{(supporting)}}}
\;\cdot\;
...
$v_l^{(t)}$, averaged across layers, and
$\mathcal{R}_i^{(t)} = w^{(t)}\mathcal{L}_i^{(t)} +
(1-w^{(t)})\mathcal{H}_i^{(t)}$ is the standard composite reward
from prior dynamic selectors (Eq.~\ref{eq:composite}). Setting
$\lambda=0$ disables the anchor and recovers the composite-reward
base ranking, yielding a clean ablation that isolates the anchor's
contribution.
